# -*- coding: utf-8 -*-
"""
Created on Fri Jul 19 08:37:56 2019
...
#  
#  encode_out_w1 = tf.Variable(tf.truncated_normal([18432,10], stddev=0.1))
#  encode_out_b1 = tf.Variable(tf.truncated_normal([10],stddev=0.1))
#  decode_in_w1 = tf.Variable(tf.truncated_normal([10,18432],stddev=0.1))
#  decode_in_b1 = tf.Variable(tf.truncated_normal([18432],stddev=0.1))
#  
...
#  a = tf.reshape(encode_layer3_out,[-1,12*12*128])
...
#  b = tf.reshape(decode_in,[-1,12,12,128])
#  
...
def creat_net(x):
  ...
  encode_out_w1 = tf.Variable(tf.truncated_normal([24*24*64,10], stddev=0.1))
  encode_out_b1 = tf.Variable(tf.truncated_normal([10],stddev=0.1))
  decode_in_w1 = tf.Variable(tf.truncated_normal([10,24*24*64],stddev=0.1))
  decode_in_b1 = tf.Variable(tf.truncated_normal([24*24*64],stddev=0.1))
  ...
  encode_layer1_out = tf.nn.sigmoid(conv2d(x, encode_layer1_w,'SAME') + encode_layer1_b)
  encode_layer2_out = tf.nn.relu(conv2d(encode_layer1_out, encode_layer2_w,'SAME') + encode_layer2_b)
  a = tf.reshape(encode_layer2_out,[-1,24*24*64])
  encode_out_nol2 = tf.add(tf.matmul(a,encode_out_w1),encode_out_b1)
...
#  print(xx)
#  print(test1)
  decode_in = tf.nn.sigmoid(tf.add(tf.matmul(encode_out,decode_in_w1),decode_in_b1))
  b = tf.reshape(decode_in,[-1,24,24,64])
  decode_layer2_out = tf.nn.relu(deconv2d(b, 32,'SAME',(5,5)) + decode_layer2_b)
  decode_layer3_out = tf.nn.sigmoid(deconv2d(decode_layer2_out, 3,'SAME',(5,5)) + decode_layer3_b)
...
def PLOT(data,reconstruct_data):
  ...
  for i in range(n):
    ...
    plt.imshow(rand_x[i].reshape(32,32,3))
    plt.gray()
    plt.axis('off')
  ...
  for i in range(n):
    ax = plt.subplot(10, 10, i + 1)
    plt.imshow(rand_y[i].reshape(32,32,3))
    plt.gray()
    plt.axis('off')
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
...
if __name__ == '__main__':
  ...
  sess = tf.Session()
  autoencoder_input = tf.placeholder(tf.float32,[None,96,96,3])
...
#  training
  ...
  for episode in range(300):
    ...
    tr_a =tr_a[dex]
    for i in range(np.int(np.shape(tr_a)[0]/32)):
      #batch_size = 128
      sess.run(train_step, feed_dict={autoencoder_input:tr_a[i*32:i*32+32]})
    #visualization
    rs=sess.run(merged,feed_dict={autoencoder_input:tr_a[i*32:i*32+32]})
#    rs2=sess.run(loss,feed_dict={autoencoder_input:tr_a[i*32:i*32+32]})
    writer.add_summary(rs, episode)
    print('epsiod {}'.format(episode))
#    print('loss {}'.format(rs2))
...
#  compress_data = np.loadtxt('compress_data.txt')
  ...
  index = np.random.choice(200,80)
  GP_init_data1 = compress_data[dis_index[index]]
  GP_init_data2 = compress_data[dis_index[np.size(dis_index)-40:np.size(dis_index)],:]
  GP_init_data = np.float64(np.vstack((GP_init_data1,GP_init_data2)))
  GP_init_label1 = np.zeros([80,1])
  GP_init_label2 = np.ones([40,1])
  GP_init_label = np.float64(np.vstack((GP_init_label1,GP_init_label2)))
  
  label1 = label[dis_index[index]]
  label2 = label[dis_index[np.size(dis_index)-40:np.size(dis_index)]]
#  np.savetxt('label1.txt',label1)
#  np.savetxt('label2.txt',label2)
#  label1 = np.loadtxt('label1.txt')
#  label2 = np.loadtxt('label2.txt')
